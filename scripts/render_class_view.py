...
parser.add_argument('-tex', '--texture', default='')
...
try:
    render_cmd = '%s %s --background --python %s -- %s %s %s %s %s %s' % (g_blender_executable_path, blank_file, render_code,args.texture, args.model_file, 'xxx', 'xxx', view_file, temp_dirname)
    print render_cmd
