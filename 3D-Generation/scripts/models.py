...
def MBD(inputs, output_size): 
    nb_kernels = 8
    kernel_dim = 4
    x = inputs
    W = tf.get_variable(
                    'w_var',
                    [nb_kernels,64*output_size,kernel_dim],
                    tf.float32,
                    tf.truncated_normal_initializer(stddev=0.02))
    activation = tf.reshape(K.dot(x, W), (-1, nb_kernels, kernel_dim))
    diffs = tf.expand_dims(activation, 3) - tf.expand_dims(tf.transpose(activation, [1, 2, 0]), 0)
    abs_diffs = K.sum(tf.abs(diffs), axis=2)
    return K.sum(tf.exp(-abs_diffs), axis=2)
