...
parfor(i = 1:image_num, num_workers)
    ...
    if rand() > clutteredBkgRatio
        ...
        up_down = randi([-200,200]);
        left_right = randi([-200,200]) ;
        J = imtranslate(I,[up_down, left_right]);
        % mask = imtranslate(mask, [up_down,left_right]); and uncomment this 
        I = uint8(double(I) .* mask + double(bgcrop) .* (1 - mask)); % change to J to keep translation 
    end
